...
In large systems or in pipelines doing reference data lookups across a wide time range this can lead to very large heap sizes.
...
As the reference data is held off-heap, _Stroom_ can make use of all available free RAM for the reference data.
...
* Toggle Vim key bindings.
  The Ace editor does not implement all Vim functionality but supports the core key bindings.
* Toggle auto-completion.
  Completion is triggered using `ctrl+space`.
* Toggle live auto-completion.
  Completion is triggered as you type.
...
It opens up the possibility for interacting with _Stroom_ via scripts or from other applications.
