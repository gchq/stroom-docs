...
- the user is deploying for either
 - the example two node Stroom cluster whose storage is described [here]({{< relref "install-how-to.md#storage-scenario" >}})
...
With respect to the working area, we will make use of the [Storage Scenario]({{< relref "install-how-to.md#storage-scenario" >}}) we have defined and hence use the directory `/stroomdata/stroom-working-p_nn_` where _nn_ is the hostname node number (i.e 00 for host stroomp00, 01 for host stroomp01, etc).
