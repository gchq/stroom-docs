...
An extract of BlueCoat logs was sourced from {{< external-link "log-sharing.dreamhosters.com" "http://log-sharing.dreamhosters.com" >}} (a Public Security Log Sharing Site) but modified to add sample user attribution.
...
They will be placed in a folder appropriately named **BlueCoat** in the path **System/Event Sources/Proxy**.
...
- the Text Converter to convert the BlueCoat raw log files into simple XML
...
{{< stroom-menu "New" "Data Processing" "Feed" >}}
...
On the `Settings` tab the configuration items for an Event Feed are
 ...
 * _Received Type_ - to indicate if the `Feed` contains raw log data or reference data
 ...
 * _Data Format_ - The format of the data part of streams received into this Feed, e.g. XML, JSON, CSV, etc.
 * _Context Format_ - The format of the context part of streams received into this Feed, e.g. XML, JSON, CSV, etc.
 * _Data Schema_ - The name of the schema that the data conforms to, if applicable
 * _Data Schema Version_ - The version of the schema that the data conforms to, if applicable
 * _Volume Group_ - The volume group that this Feed's data will be stored in
 ...
 * _Received Type_ - We set to *Raw Events* as we will be sending batches (streams) of raw event logs
 ...
 * _Data Format_, _Context Format_, _Data Schema_, _Data Schema Version_ and _Volume Group_ will all be left blank

{{< screenshot "HOWTOs/UI-FeedProcessing-02.png" >}}Stroom UI Create Feed - New feed tab configuration{{< /screenshot >}}

And add *BlueCoat Proxy log data sent in W2C Extended Log File Format (ELFF)* to the `Documentation` hyperlink

{{< screenshot "HOWTOs/UI-FeedProcessing-02a.png" >}}Stroom UI Create Feed - Documentation{{< /screenshot >}}
...
On the `Settings` tab we set the configuration for this `Text Converter` to be
...
And add *Simple XML transform for BlueCoat Proxy log data sent in W2C Extended Log File Format (ELFF)* to the `Documentation` hyperlink
...
{{< stroom-menu "New" "Transformation" "XSL Translation" >}}
...
Click on the `Documentation` hyperlink and enter *Transform simple XML of BlueCoat Proxy log data into Stroom Event Logging XML form*
...
On the `Documentation` hyperlink we enter *Processing of XML of BlueCoat Proxy log data into Stroom Event Logging XML*
...
We do this by selecting the `Structure` tab of the **BlueCoat-Proxy-V1.0-EVENTS** `Pipeline`
...
As noted in the Assumptions at the start, we have loaded the **Template Pipelines** content pack, so that we can _Inherit_ a pipeline structure from this content pack and configure it to support this specific feed.
...
This is done by moving the cursor to the relevant line and selecting via a _left click_
...
At this point, the `Edit Property` configuration window is displayed.
...
We select the **Value:** _None_ {{< stroom-icon "ellipses-horizontal.svg" "Menu selection" >}} entry box to reveal a **Choose Item** configuration item window.
...
{{< screenshot "HOWTOs/UI-FeedProcessing-21.png" >}}Stroom UI Create Feed - Pipeline Structure tab - XSLT Property selected{{< /screenshot >}}
...
On the **BlueCoat-Proxy-V1.0-EVENTS** `Feed` tab, select the **Data** hyperlink to be presented with the **Data** pane of our tab.
...
MyTZ:+1000
SHA256:056f0d196ffb4bc6c5f3898962f1708886bb48e2f20a81fb93f561f4d16cb2aa
...
If we were to select the **Meta** hyperlink of the lower pane, one would see the metadata Stroom records for this Stream of data.
...
We now switch back to the **Data** hyperlink before we start to develop the actual translation.
...
We start by ensuring our `Raw Events` Data stream is selected, and we press the `Enter Stepping Mode` {{< stroom-icon "stepping-circle.svg" >}} button on the lower right hand side of the bottom `Stream Data` pane.

You will be prompted to select a pipeline to step with. 
At this point it is possible to create a new pipeline by clicking {{< stroom-btn "new" >}} but we will choose the `BlueCoat-Proxy-V1.0-EVENTS` pipeline
...
This will open the pipeline in _Stepping Mode_.
...
We see three panes here.

The top pane displays the Pipeline structure with `Source` selected, and it also displays a _step indicator_ (three colon separated numbers enclosed in square brackets initially the numbers are dashes i.e. `[-:-:-]` as we have yet to step) and a set of green _Stepping Actions_.
The step indicator and Stepping Actions allows one to _step through_ a log file, selecting data event by event (an event is typically a line, but some events can be multi-line).

The middle pane displays a meta list table. 
When the user enters Stepping Mode using the `Enter Stepping Mode` {{< stroom-icon "stepping-circle.svg" >}} button then the table will be filtered to only show the stream that was selected.
The user can find different data to test by pressing the {{< stroom-icon "filter.svg" "Filter" >}} button in the top left of the pane. 
The source data being tested can be changed at any time by selecting a different input stream in the table. 
...
The right hand one will allow a user to step through the source data as before, but the left hand set of control buttons allows one to step between files from the aggregated event log files.
...
This window is made up of five panes.
...
The bottom pane will show any errors the selected pipeline element has produced processing the data.
...
So, if one was to press the {{< stroom-icon name="refresh.svg" title="Refresh step" colour="green" >}} _stepping action_ we would be presented with
...
If one presses the {{< stroom-icon name="step-forward.svg" title="Step forward" colour="green" >}} _stepping action_, we see that we have moved to the second line of the input file with the resultant output of a **data** element with the _name_ attribute of _bc_version_ and its _value_ attribute of _1.0_.
...
You will note that the keys have been taken from the Fields comment line which were placed in the $headings variable.
...
Clicking the _stepping indicator_ will allow you to edit any of these values.
This allows you to _jump_ around a stream rather than just relying on first, previous, next and last movements.

{{< screenshot "HOWTOs/UI-FeedProcessing-36a.png" >}}Stroom UI Create Feed - Translation - Edit Stepping Location{{< /screenshot >}}

If we change the record number from __3__ to __12__ and press OK then a new record will have been processed in the _input_ and _output_ panes.
Further, if one goes back to the `Source` element of the pipeline to view the raw source file, we see that the highlighted __current__ line is the 14th line of processed data.
It is the 10th actual BlueCoat event, but remember the #Software, #Version lines are considered as processed data (2+12 = 14).
...
{{< screenshot "HOWTOs/UI-FeedProcessing-39.png" >}}Stroom UI Create Feed - Translation - Stepping Indicator 1{{< /screenshot >}}
...
{{< screenshot "HOWTOs/UI-FeedProcessing-40.png" >}}Stroom UI Create Feed - Translation - Stepping Indicator 2{{< /screenshot >}}
...
You will note that the _input_ and _output_ displays are identical for a null XSLT translation, which is effectively a direct copy.
...
Note that this is the 31st record, so if we were to jump to the first record using the {{< stroom-icon name="fast-backward.svg" title="Step first" colour="green" >}} action, we would see that the _input_ and _output_ change appropriately.
...
Stepping to the 3rd record (the first real data record in our sample log) will reveal that our _output_ pane has gained an `EventSource` element.

{{< screenshot "HOWTOs/UI-FeedProcessing-46.png" >}}Stroom UI Create Feed - Translation - Stepping XSLT Translation 4{{< /screenshot >}}
...
* The `schemaFilter` pipeline element in the structure pane has been outlined in red indicating there was an error processing the data

* There is an error pane at the bottom of the window describing the issue 

{{< screenshot "HOWTOs/UI-FeedProcessing-47.png" >}}Stroom UI Create Feed - Translation - Stepping XSLT Translation 5{{< /screenshot >}}
...
{{< screenshot "HOWTOs/UI-FeedProcessing-49.png" >}}Stroom UI Create Feed - Translation - Stepping XSLT Translation 6{{< /screenshot >}}
...
To configure a Processor we select the `Processors` hyperlink of the **BlueCoat-Proxy-V1.0-EVENTS** `Pipeline` tab to reveal
...
We will create a Processor that will automatically process all **BlueCoat-Proxy-V1.0-EVENTS** feed `Raw Events`.

To select the feed, we press the `Add Term` button {{< stroom-icon "add.svg" >}} in the `Expression` pane.

{{< screenshot "HOWTOs/UI-FeedProcessing-53.png" >}}Stroom UI Enable Processing - Add Filter 2{{< /screenshot >}}

Click _None_ {{< stroom-icon "ellipses-horizontal.svg" "Menu selection">}} to show the `Choose Item` dialog.
...
Press the {{< stroom-icon "add.svg" >}} button again in the `Expression` pane of the window and select `Type` from the left hand side drop down.

{{< screenshot "HOWTOs/UI-FeedProcessing-58.png" >}}Stroom UI Enable Processing - Add Filter 7{{< /screenshot >}}

We select (_left click_) the dropdown selection to display the types of Stream we can choose

{{< screenshot "HOWTOs/UI-FeedProcessing-59.png" >}}Stroom UI Enable Processing - Add Filter 8{{< /screenshot >}}

and as we are selecting `Raw Events` we select that item.
...
We now see our display is divided into three panes.
The Processors table pane at the top and the specific Processor information panes below.
...
and the specific filter's details in the bottom two panes.
 ...
 * Reprocess - ???
 * Run As User - The user that will run the stream processor filter tasks 
...
We see that the last time active streams were checked was _1.0_ seconds ago and that _1_ stream has completed.

If we return to the **BlueCoat-Proxy-V1.0-EVENTS** `Feed` tab, ensuring the **Data** hyperlink is selected and then refresh ({{< stroom-icon "refresh.svg">}}) the top pane that holds the summary of the latest Feed streams
...
If we select (_left click_) the `Raw Events` stream in the top pane we see the stream and its related `Events` _Type_ entry displayed in the middle pane
...
We can now send a file of BlueCoat Proxy logs to our Stroom instance from a Linux host using the _curl_ command and see how Stroom will automatically process the file.
Use the command
...
If we switch to the `Processors` tab of the pipeline we see that the number of Streams processed has increased.
