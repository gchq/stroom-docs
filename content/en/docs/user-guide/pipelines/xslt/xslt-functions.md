...
* [`lookup-geoip2(String ipAddress, String feedName, String streamType, String time)`](#lookup-geoip2) - Look up location information about an IP address, using a Maxmind GeoIP2 database in a reference feed.
* [`lookup-geoip2(String ipAddress, String feedName, String streamType, String time, Boolean ignoreErrors)`](#lookup-geoip2) - Look up location information about an IP address, using a Maxmind GeoIP2 database in a reference feed. Ignore any IP address lookup errors.
...
The `lookup-geoip2()` function looks up location information from an IP address (such as country and city), against a Maxmind GeoIP2 database.
The database is expected to be stored in binary form in a reference feed, as with regular Stroom reference data.

The GeoLite2 City database is (at the time of writing) freely available from [Maxmind](https://dev.maxmind.com/geoip/geolite2-free-geolocation-data).
It is recommended that updates to this database be posted to a Stroom reference feed, to ensure lookups are performed against current data.

Stroom maintains a cache named `Maxmind GeoIP2 Database Reader Cache` that persists the database lookup client, for performance reasons.
To increase the number of cache hits, it is recommended that the `time` parameter be rounded to the nearest day.

### Usage

```text
lookup-geoip2(String ipAddress, String feedName, String streamType, String time)
lookup-geoip2(String ipAddress, String feedName, String streamType, String time, Boolean ignoreErrors)
```

* `ipAddress` - The IPv4 address to look up information against.
* `feedName` - Name of the feed containing Maxmind GeoIP2/GeoLite2 database reference streams.
* `streamType` - Typically `Raw Reference`. This must match the type of the streams in the reference feed.
* `time` - Determines which set of reference data was effective at the requested time.
  If no reference data exists with an effective time before the requested time then the lookup will fail.
  Time is in the format `yyyy-MM-dd'T'HH:mm:ss.SSSXX`, e.g. `2010-01-01T00:00:00.000Z`.
* `ignoreErrors` - If true, any GeoIP2 lookup failures will be suppressed. 
  Any other types of errors, such as argument parse errors, will be reported as `Error` streams.

### Output

The output from this function is JSON XML. The actual format depends on the type of database stored (i.e. GeoLite2 or the commercial GeoIP2 version).
