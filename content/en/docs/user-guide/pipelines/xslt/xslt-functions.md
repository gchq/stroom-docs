...
* [`cidr-to-numeric-ip-range(String cidr)`](#cidr-to-numeric-ip-range) - Converts a CIDR IP address range to an array of numeric IP addresses representing the start and end addresses of the range.
...
* [`cosine-similarity(Number[] vectorA, Number[] vectorB)`](#cosine-similarity) - Returns the cosine similarity of two numeric vectors, e.g. two sets of embeddings produced by a large language model.
...
* [`current-user(String form)`](#current-user) - As above, where `form` selects which form of the user's identity is returned.
...
* [`format-date(String date, String patternIn, String timeZoneIn, String patternOut)`](#format-date) - Parse a date with the specified input pattern and time zone and format the output with the specified output pattern
...
* [`http-call(String url)`](#http-call) - Makes an HTTP(S) GET request to a remote server.
* [`http-call(String url, String headers, String mediaType, String data, String clientConfig)`](#http-call) - Makes an HTTP(S) request to a remote server, where all arguments after `url` are optional.
...
* [`meta-attribute(String key)`](#meta-attribute) - Lookup a single attribute from the manifest of the current stream, i.e. one entry of the XML structure returned by `manifest()`.
...
* `split-document(String doc, Number segmentSize, Number overlapSize)` - Split a document for LLM tokenisation (experimental).
